...
static int32_t migrateDataFile(SRTNer* rtner, STFileObj* fobj) {
  ...
  code = taosStatFile(fobj->fname, &szFile, NULL, NULL);
  if (code != TSDB_CODE_SUCCESS) {
    tsdbError("vgId:%d, failed to stat file %s since %s", vid, fobj->fname, tstrerror(code));
    return false;
  }
  ...
  for (int i = lcn; i <= totalChunks; ++i) {
    ...
    char path[TSDB_FILENAME_LEN];
    if (i == totalChunks) {
      sprintf(path, "vnode%d/f%d/v%df%dver%" PRId64 ".%d.data.%d", vid, f->fid, vid, f->fid, f->cid, i, ++f->mcount);
    } else {
      sprintf(path, "vnode%d/f%d/v%df%dver%" PRId64 ".%d.data", vid, f->fid, vid, f->fid, f->cid, i);
    }

    code = tssUploadFileToDefault(path, fobj->fname, offset, size);
    if (code != TSDB_CODE_SUCCESS) {
  ...
  TdFilePtr fdFrom = taosOpenFile(fobj->fname, TD_FILE_READ);
  if (fdFrom == NULL) {
    code = terrno;
    tsdbError("vgId:%d, failed to open file %s since %s", vid, fobj->fname, tstrerror(code));
    return code;
  }

  TdFilePtr fdTo = taosOpenFile(path, TD_FILE_WRITE | TD_FILE_CREATE | TD_FILE_TRUNC);
  if (fdTo == NULL) {
    code = terrno;
  ...
  if (n < 0) {
    code = terrno;
    tsdbError("vgId:%d, failed to copy file %s to %s since %s", vid, fobj->fname, path, tstrerror(code));
  }
  taosCloseFile(&fdFrom);
  taosCloseFile(&fdTo);
  if (code != TSDB_CODE_SUCCESS) {
...
static bool shouldMigrate(SRTNer *rtner, int32_t *pCode) {
  ...
  *pCode = taosStatFile(flocal->fname, &size, &mtime, NULL);
  if (*pCode != TSDB_CODE_SUCCESS) {
    tsdbError("vgId:%d, failed to stat file %s since %s", vid, flocal->fname, tstrerror(*pCode));
    return false;
  }
